  ...
  return (
    <SolutionLayout title="Строка">
      <form className={styles.input_container} onSubmit={onSubmit}>
        <Input maxLength={11} height={60} type='text' onChange={onChange} isLimitText={true} autoFocus/>
        <Button height={60} type='submit' text='Развернуть' isLoader={isLoading} />
      </form>

      <div className={styles.result_container}>
        {lettersArray && lettersArray.map((item, index) => (
          <Circle key={index} letter={item} state={circleStyle(index, modifiedIndexes, changingIndexes)} />
